...
class APIBenchmark:
    ...
    async def authenticate(self) -> str:
        ...
        async with (
            ...
            session.post(f"{self.base_url}/api/v1/auth/token", data=login_data) as response,
        ):
    ...
    async def run_benchmarks(self, concurrent_requests: int = 10, total_requests: int = 100):
        ...
        for method, endpoint, *kwargs in endpoints:
            ...
            if (
                ...
                and endpoint not in ["/api/v1/auth/token"]
    ...
    def _print_summary(self):
        ...
        for i, result in enumerate(fastest, 1):
            print(
                f"  {i}. {result.method} {result.endpoint}: {result.avg_response_time * 1000:.2f}ms"
            )
...
async def main():
    ...
    try:
        await benchmark.run_benchmarks(
            concurrent_requests=args.concurrent, total_requests=args.total
        )
