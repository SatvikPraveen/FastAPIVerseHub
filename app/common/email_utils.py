...
class EmailService:
    ...
    async def send_course_enrollment_confirmation(
        self, email: str, course_title: str, instructor_name: str, name: str | None = None
    ) -> bool:
        ...
        return await self.send_email(
            to_emails=[email], subject=f"Enrolled in {course_title}", html_body=html_body
        )

    async def send_course_completion_certificate(
        self, email: str, course_title: str, certificate_url: str, name: str | None = None
    ) -> bool:
        ...
        return await self.send_email(
            to_emails=[email], subject=f"Certificate for {course_title}", html_body=html_body
        )
    ...
    async def send_notification_email(
        ...
        return await self.send_email(to_emails=[email], subject=subject, html_body=html_body)
        ...
        for i in range(0, len(recipients), batch_size):
            ...
            try:
                success = await self.send_email(
                    to_emails=batch, subject=subject, html_body=html_body, text_body=text_body
                )
