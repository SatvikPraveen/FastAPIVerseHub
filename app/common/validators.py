...
class ValidationUtils:
    ...
    def validate_phone(phone: str, country_code: str | None = None) -> bool:
        ...
        if country_code == "US":
            return len(clean_phone) == 10 or (
                len(clean_phone) == 11 and clean_phone.startswith("1")
            )
        else:
...
def file_size_validator(max_size: int):
    ...
    def validator_func(cls, v):
        if getattr(v, "size", None) and not ValidationUtils.validate_file_size(v.size, max_size):
...
def file_extension_validator(allowed_extensions: list[str]):
    ...
    def validator_func(cls, v):
        if getattr(v, "filename", None) and not ValidationUtils.validate_file_extension(
            v.filename, allowed_extensions
        ):
            raise ValueError(
