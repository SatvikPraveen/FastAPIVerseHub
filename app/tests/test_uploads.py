...
class TestFileUpload:
    ...
    async def test_upload_single_file_success(
        self, async_client: AsyncClient, auth_headers, mock_file_manager
    ):
        ...
        data = {"description": "Test file upload", "category": "documents", "is_public": "false"}

        with patch("app.api.v1.uploads.FileManager", return_value=mock_file_manager):
            response = await async_client.post(
                "/api/v1/uploads/file", files=files, data=data, headers=auth_headers
            )
    ...
    async def test_upload_file_too_large(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.common.file_utils.FileManager.validate_file") as mock_validate:
            ...
            response = await async_client.post(
                "/api/v1/uploads/file", files=files, headers=auth_headers
            )
    ...
    async def test_upload_invalid_file_type(self, async_client: AsyncClient, auth_headers):
        ...
        files = {"file": ("malware.exe", BytesIO(b"malicious"), "application/executable")}
    ...
    async def test_upload_multiple_files_success(
        ...
        data = {"description": "Multiple test files", "category": "documents"}

        with patch("app.api.v1.uploads.FileManager", return_value=mock_file_manager):
            response = await async_client.post(
                "/api/v1/uploads/multiple", files=files, data=data, headers=auth_headers
            )
    ...
    async def test_upload_too_many_files(self, async_client: AsyncClient, auth_headers):
        ...
        response = await async_client.post(
            "/api/v1/uploads/multiple", files=files, headers=auth_headers
        )
...
class TestFileDownload:
    ...
    async def test_download_file_success(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            with (
                ...
                patch("app.services.file_service.FileService.increment_download_count"),
            ):
                response = await async_client.get(
                    "/api/v1/uploads/download/1", headers=auth_headers
                )
    ...
    async def test_download_file_not_found(self, async_client: AsyncClient, auth_headers):
        """Test download of nonexistent file."""
        with patch("app.services.file_service.FileService.get_file_by_id", return_value=None):
            response = await async_client.get("/api/v1/uploads/download/999", headers=auth_headers)

        assert_response_error(response, 404, "NOT_FOUND")

    async def test_download_file_access_denied(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            response = await async_client.get("/api/v1/uploads/download/1", headers=auth_headers)

        assert_response_error(response, 403, "FORBIDDEN")

    async def test_download_public_file(self, async_client: AsyncClient):
...
class TestFileStreaming:
    ...
    async def test_stream_file_success(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            with (
                patch("os.path.exists", return_value=True),
                patch("app.common.file_utils.FileManager.stream_file") as mock_stream,
            ):
                mock_stream.return_value = [b"chunk1", b"chunk2"]

                response = await async_client.get("/api/v1/uploads/stream/1", headers=auth_headers)
...
class TestFileManagement:
    ...
    async def test_list_user_files(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_user_files") as mock_get_files:
            ...
            response = await async_client.get("/api/v1/uploads/", headers=auth_headers)
    ...
    async def test_get_file_info(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            response = await async_client.get("/api/v1/uploads/file/1", headers=auth_headers)
    ...
    async def test_update_file_info(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            with patch("app.services.file_service.FileService.update_file_info") as mock_update:
                ...
                response = await async_client.put(
                    "/api/v1/uploads/file/1", data=update_data, headers=auth_headers
                )

        assert_response_success(response)

    async def test_delete_file_success(self, async_client: AsyncClient, auth_headers):
            ...
            with patch("app.common.file_utils.FileManager.delete_file") as mock_delete:
                ...
                with patch("app.services.file_service.FileService.delete_file"):
                    response = await async_client.delete(
                        "/api/v1/uploads/file/1", headers=auth_headers
                    )

        assert_response_success(response, 204)

    async def test_delete_file_not_owner(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_file_by_id") as mock_get_file:
            ...
            response = await async_client.delete("/api/v1/uploads/file/1", headers=auth_headers)
...
class TestFileStatistics:
    ...
    async def test_get_file_categories(self, async_client: AsyncClient, auth_headers):
        """Test getting file categories with counts."""
        with patch(
            "app.services.file_service.FileService.get_categories_with_counts"
        ) as mock_get_cats:
            ...
            response = await async_client.get("/api/v1/uploads/categories", headers=auth_headers)
    ...
    async def test_get_file_stats(self, async_client: AsyncClient, auth_headers):
        ...
        with patch("app.services.file_service.FileService.get_user_file_stats") as mock_get_stats:
            ...
            response = await async_client.get("/api/v1/uploads/stats", headers=auth_headers)
