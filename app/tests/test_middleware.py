...
class TestRateLimitMiddleware:
    ...
    async def test_rate_limit_exceeded(self, async_client: AsyncClient):
        ...
        responses = []
        for _i in range(100):  # Exceed typical rate limit
            response = await async_client.get("/")
...
class TestRequestTimingMiddleware:
    ...
    async def test_slow_request_logging(self, async_client: AsyncClient, caplog):
        ...
        with (
            patch.object(request_timer, "SLOW_REQUEST_THRESHOLD_SECONDS", 0.0),
            caplog.at_level("WARNING", logger="app.middleware.request_timer"),
        ):
            response = await async_client.get("/health")
...
class TestMiddlewareIntegration:
    ...
    async def test_middleware_with_authentication(self, async_client: AsyncClient, auth_headers):
