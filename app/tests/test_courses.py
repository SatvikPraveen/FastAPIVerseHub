...
class TestCourseCreation:
    ...
    async def test_create_course_success(
        self, async_client: AsyncClient, sample_course_data, auth_headers
    ):
        """Test successful course creation."""
        response = await async_client.post(
            "/api/v1/courses/", json=sample_course_data, headers=auth_headers
        )
    ...
    async def test_create_course_unauthorized(self, async_client: AsyncClient, sample_course_data):
    ...
    async def test_create_course_invalid_data(self, async_client: AsyncClient, auth_headers):
        ...
        response = await async_client.post(
            "/api/v1/courses/", json=invalid_data, headers=auth_headers
        )
    ...
    async def test_create_course_with_pricing(
        ...
        sample_course_data.update({"price": 199.99, "original_price": 299.99, "is_free": False})
...
class TestCourseRetrieval:
    ...
    async def test_get_courses_with_pagination(
        ...
        for i in range(5):
            await CourseFactory.create_course(
                db_session, test_user.id, title=f"Course {i}", is_published=True
            )
    ...
    async def test_get_courses_with_filters(self, async_client: AsyncClient, test_course: Course):
...
class TestCourseUpdating:
    ...
    async def test_update_course_success(
        self, async_client: AsyncClient, test_course: Course, auth_headers
    ):
        ...
        response = await async_client.put(
            f"/api/v1/courses/{test_course.id}", json=update_data, headers=auth_headers
        )
    ...
    async def test_update_course_unauthorized(self, async_client: AsyncClient, test_course: Course):
        """Test course update without authentication."""
        update_data = {"title": "Unauthorized Update"}

        response = await async_client.put(f"/api/v1/courses/{test_course.id}", json=update_data)

        assert_response_error(response, 401, "UNAUTHORIZED")

    async def test_update_course_not_owner(
        self, async_client: AsyncClient, test_course: Course, db_session: AsyncSession
    ):
        """Test course update by non-owner."""
        # Create different user
        other_user = await UserFactory.create_user(db_session, email="other@example.com")

        other_headers = {"Authorization": f"Bearer {create_test_token(other_user.id)}"}

        update_data = {"title": "Unauthorized Update"}

        response = await async_client.put(
            f"/api/v1/courses/{test_course.id}", json=update_data, headers=other_headers
        )

        assert_response_error(response, 403, "FORBIDDEN")

    async def test_update_course_not_found(self, async_client: AsyncClient, auth_headers):
        """Test updating nonexistent course."""
        update_data = {"title": "Updated Title"}

        response = await async_client.put(
            "/api/v1/courses/999999", json=update_data, headers=auth_headers
        )
...
class TestCourseDeletion:
    ...
    async def test_delete_course_success(
        self, async_client: AsyncClient, db_session: AsyncSession, test_user: User, auth_headers
    ):
        """Test successful course deletion."""
        # Create a course to delete
        course_to_delete = await CourseFactory.create_course(
            db_session, test_user.id, title="Course to Delete"
        )

        response = await async_client.delete(
            f"/api/v1/courses/{course_to_delete.id}", headers=auth_headers
        )

        assert_response_success(response, 204)

    async def test_delete_course_unauthorized(self, async_client: AsyncClient, test_course: Course):
    ...
    async def test_delete_course_not_owner(
        ...
        response = await async_client.delete(
            f"/api/v1/courses/{test_course.id}", headers=other_headers
        )
...
class TestCoursePublishing:
    ...
    async def test_publish_course_success(
        ...
        unpublished_course = await CourseFactory.create_course(
            db_session, test_user.id, is_published=False
        )

        response = await async_client.post(
            f"/api/v1/courses/{unpublished_course.id}/publish", headers=auth_headers
        )
    ...
    async def test_unpublish_course_success(
        ...
        response = await async_client.post(
            f"/api/v1/courses/{test_course.id}/unpublish", headers=auth_headers
        )
    ...
    async def test_publish_already_published_course(
        ...
        response = await async_client.post(
            f"/api/v1/courses/{test_course.id}/publish", headers=auth_headers
        )
...
class TestCourseEnrollment:
    ...
    async def test_enroll_in_course_success(
        ...
        student = await UserFactory.create_user(db_session, email="student@example.com")

        student_headers = {"Authorization": f"Bearer {create_test_token(student.id)}"}

        response = await async_client.post(
            f"/api/v1/courses/{test_course.id}/enroll", headers=student_headers
        )
    ...
    async def test_enroll_in_unpublished_course(
        ...
        response = await async_client.post(
            f"/api/v1/courses/{unpublished_course.id}/enroll", headers=student_headers
        )
    ...
    async def test_unenroll_from_course_success(
        ...
        # First enroll
        await async_client.post(f"/api/v1/courses/{test_course.id}/enroll", headers=student_headers)
...
class TestCourseCategories:
    ...
    async def test_get_course_categories(self, async_client: AsyncClient, test_course: Course):
