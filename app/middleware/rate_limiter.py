    ...
    async def _check_rate_limits(self, client_id: str, request: Request) -> dict:
        ...
        for limit_config in limits:
            is_limited, info = await self.rate_limit_cache.is_rate_limited(
                limit_config["key"], limit_config["limit"], limit_config["window"]
            )
        ...
        _, info = await self.rate_limit_cache.is_rate_limited(
            minute_limit["key"], minute_limit["limit"], minute_limit["window"]
        )
...
class EndpointRateLimiter:
    ...
    async def check_endpoint_limit(self, endpoint: str, client_id: str) -> tuple[bool, dict]:
        ...
        if not is_limited:
            ...
            if current_count >= config["calls"]:
                return True, {"limit": config["calls"], "remaining": 0, "window": config["window"]}
