...
async def setup_mfa(
    ...
    # Generate QR code
    provisioning_uri = totp.provisioning_uri(name=current_user.email, issuer_name="FastAPIVerseHub")
    ...
    await auth_service.setup_mfa(user_id=current_user.id, secret=secret, backup_codes=backup_codes)
...
async def social_auth(
    ...
    if not user_info:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid social authentication"
        )
...
async def request_passwordless_auth(
    request: PasswordlessRequest, db: AsyncSession = Depends(get_db)
) -> dict[str, str]:
...
async def verify_magic_link(
    ...
    try:
        ...
        return {**tokens, "user": {"id": user.id, "email": user.email, "full_name": user.full_name}}
