...
async def get_course_recommendations(
    ...
    recommendations = await ai_service.generate_course_recommendations(
        user_data=user_data, limit=limit, include_reasoning=include_reasoning
    )
...
async def get_learning_paths(
    ...
    learning_paths = await advanced_course_service.get_learning_paths(
        category=category, difficulty=difficulty, user_id=current_user.id
    )
...
async def create_custom_learning_path(
    ...
    saved_path = await advanced_course_service.save_learning_path(
        user_id=current_user.id, learning_path_data=learning_path
    )

    # Send notification about new learning path
    background_tasks.add_task(
        advanced_course_service.send_learning_path_notification, current_user.id, saved_path.id
    )
...
async def get_course_analytics(
    ...
    analytics = await analytics_service.get_course_analytics(
        course_id=course_id, time_range=time_range
    )
...
async def get_performance_dashboard(
    ...
    dashboard_data = await analytics_service.get_instructor_dashboard(
        instructor_id=current_user.id, time_range=time_range
    )
...
async def optimize_course(
    ...
    report_id = await advanced_course_service.save_optimization_report(
        course_id=course_id, user_id=current_user.id, optimization_results=optimization_results
    )
...
async def get_market_trends(
    ...
    trends = await analytics_service.get_market_trends(category=category, time_range=time_range)
...
async def get_course_cohorts(
    ...
    cohorts = await advanced_course_service.get_course_cohorts(
        course_id=course_id, skip=pagination.skip, limit=pagination.limit
    )
...
async def create_ab_test(
    ...
    experiment = await advanced_course_service.create_ab_experiment(
        course_id=course_id, instructor_id=current_user.id, config=experiment_config
    )
...
async def bulk_update_courses(
    ...
    valid_courses = await advanced_course_service.validate_instructor_courses(
        course_ids=course_ids, instructor_id=current_user.id
    )

    if len(valid_courses) != len(course_ids):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Some courses are not accessible"
        )

    # Schedule bulk update task
    background_tasks.add_task(
        advanced_course_service.bulk_update_courses, course_ids, updates, current_user.id
    )
