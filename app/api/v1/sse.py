...
async def sse_endpoint(
    ...
    last_event_id: str | None = Query(None, description="Last event ID for resuming stream"),
...
async def publish_to_channel(
    ...
    return {"message": f"Published to channel {channel}", "event_id": event_data["id"]}
...
async def notify_user_sse(
    ...
    return {
        ...
        if sent
        else f"User {user_id} not subscribed",
...
async def broadcast_message(
    message: dict,
    channels: str | None = Query(None, description="Comma-separated list of channels"),
    current_user: User = Depends(get_current_user_optional),
) -> dict:
...
async def clear_channel(
    channel: str, current_user: User = Depends(get_current_user_optional)
) -> dict:
    ...
    return {"message": f"Cleared channel {channel}", "messages_cleared": cleared}
