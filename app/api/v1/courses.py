...
from app.schemas.course import CourseCreate, CourseResponse, CourseUpdate, CourseWithStats
...
async def create_course(
    ...
    course = await course_service.create_course(
        course_create=course_create, instructor_id=current_user.id
    )
...
async def get_course(
    ...
    if not course.is_published and (
        not current_user
        or (current_user.id != course.instructor_id and not current_user.is_superuser)
    ):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
...
async def update_course(
    ...
    updated_course = await course_service.update_course(
        course_id=course_id, course_update=course_update
    )
...
async def publish_course(
    ...
    if course.is_published:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Course is already published"
        )
...
async def unpublish_course(
    ...
    if not course.is_published:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Course is already unpublished"
        )
...
async def enroll_in_course(
    ...
    if not course.is_published:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot enroll in unpublished course"
        )
...
async def unenroll_from_course(
    ...
    if not is_enrolled:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Not enrolled in this course"
        )
...
async def get_course_categories(db: AsyncSession = Depends(get_db)) -> list[dict[str, Any]]:
