...
async def get_current_user_ws(
    websocket: WebSocket, token: str | None = Query(None), db: AsyncSession = Depends(get_db)
) -> int | None:
...
async def handle_websocket_message(
    websocket: WebSocket, client_id: str, user_id: int | None, message: dict, db: AsyncSession
):
    ...
    if message_type == "ping":
        ...
        if channel:
            ...
            await websocket.send_json(
                {"type": "subscribed", "channel": channel, "message": f"Subscribed to {channel}"}
            )
        ...
        if not user_id:
            await websocket.send_json(
                {"type": "error", "message": "Authentication required for broadcasting"}
            )
            return
        ...
        if target_user_id:
            ...
            if sent:
                await websocket.send_json(
                    {"type": "message_sent", "message": "Private message sent successfully"}
                )
            else:
                await websocket.send_json({"type": "error", "message": "Target user not connected"})
        ...
        if room_id:
            ...
            await websocket.send_json(
                {"type": "joined_room", "room_id": room_id, "message": f"Joined room {room_id}"}
            )
        ...
        if room_id:
            ...
            await websocket.send_json(
                {"type": "left_room", "room_id": room_id, "message": f"Left room {room_id}"}
            )

    elif message_type == "room_message":
        if not user_id:
            await websocket.send_json(
                {"type": "error", "message": "Authentication required for room messages"}
            )
            return
