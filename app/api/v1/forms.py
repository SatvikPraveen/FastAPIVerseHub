...
async def submit_contact_form(
    contact_form: ContactForm, db: AsyncSession = Depends(get_db)
) -> dict[str, str]:
    """Submit contact form."""
...
async def submit_multipart_form(
    ...
    if not 13 <= age <= 120:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Age must be between 13 and 120"
        )
    ...
    if profile_picture:
        ...
        saved_file = await form_service.save_uploaded_file(
            profile_picture, current_user.id, "profile_pictures"
        )
    ...
    if resume:
        ...
        # Save resume
        saved_file = await form_service.save_uploaded_file(resume, current_user.id, "resumes")
...
async def submit_dynamic_form(
    ...
    if not form_data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Form data cannot be empty"
        )
    ...
    submission = await form_service.save_dynamic_form(
        user_id=current_user.id, form_type=form_type, form_data=form_data
    )
...
async def get_user_submissions(
    ...
    submissions, total = await form_service.get_user_submissions(
        user_id=current_user.id, skip=skip, limit=limit, form_type=form_type
    )
...
async def get_active_surveys(db: AsyncSession = Depends(get_db)) -> dict:
...
async def get_survey_details(survey_id: int, db: AsyncSession = Depends(get_db)) -> dict:
