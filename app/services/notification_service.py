    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "authenticated_clients": sum(
                1 for client in self.clients.values() if client.get("user_id")
            ),
...
class NotificationService:
    ...
    async def send_notification(
        self, user_id: int, notification: dict[str, Any], channels: list[str] | None = None
    ) -> bool:
    ...
    async def broadcast_announcement(
        self, message: str, title: str = "Announcement", channels: list[str] | None = None
    ):
    ...
    async def send_course_update(
        self, course_id: int, update_type: str, message: str, enrolled_users: list[int]
    ):
